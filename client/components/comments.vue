<template lang="pug">
  div(v-intersect.once='onIntersect')
    v-textarea#discussion-new(
      ...
      v-model='newcomment'
      color='#ffffff'
      :background-color='$vuetify.theme.dark ? `grey darken-5` : `#474759`'
    ...
    .d-flex.align-center.pt-3(v-if='permissions.write')
      v-icon.mr-1(color='white') mdi-language-markdown-outline
      .caption.white--text {{$t('common:comments.markdownFormat')}}
      v-spacer
      .caption.white--text.mr-3(v-if='isAuthenticated')
    ...
    .pa-5.d-flex.align-center.justify-center(v-if='isLoading && !hasLoadedOnce')
      ...
      .caption.white--text.pl-3: em {{$t('common:comments.loading')}}
    v-timeline(
      dense
      ...
      v-timeline-item.comments-post(
        ...
        v-card.elevation-1(color='#464F69')
          v-card-text
            .comments-post-actions(v-if='permissions.manage && !isBusy && commentEditId === 0')
              v-icon.mr-3(small, @click='editComment(cm)', color='white') mdi-pencil
              v-icon(small, @click='deleteCommentConfirm(cm)' color='white') mdi-delete
            .comments-post-name.caption.white--text: strong {{cm.authorName}}
            ...
            .comments-post-content.white--text.mt-3(v-if='commentEditId !== cm.id', v-html='cm.render')
            .comments-post-editcontent.mt-3(v-else)
              v-textarea#comment-edit(
                outlined
                flat
                auto-grow
                dense
