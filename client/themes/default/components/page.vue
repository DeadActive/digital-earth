<template lang="pug">
  v-app(v-scroll='upBtnScroll', :dark='$vuetify.theme.dark', :class='$vuetify.rtl ? `is-rtl` : `is-ltr`')
    ...
    v-navigation-drawer(
      ...
      :class='$vuetify.theme.dark ? `grey darken-4-d4` : `blue-darkest`'
      dark
      app
      clipped
      ...
      vue-scroll(:ops='scrollStyle')
        nav-sidebar(:color='$vuetify.theme.dark ? `grey darken-4-d4` : `blue-darkest`', :items='sidebarDecoded', :nav-mode='navMode')
    ...
    v-main(ref='content')
      template(v-if='path !== `home`')
        v-toolbar(:color='$vuetify.theme.dark ? `grey darken-4-d3` : `blue-light`', flat, dense, v-if='$vuetify.breakpoint.smAndUp')
          //- v-btn.pl-0(v-if='$vuetify.breakpoint.xsOnly', flat, @click='toggleNavigation')
          //-   v-icon(color='grey darken-2', left) menu
          //-   span Navigation
          v-breadcrumbs.white--text.breadcrumbs-nav.pl-0(
            :items='breadcrumbs'
            divider='/'
            )
            template(slot='item', slot-scope='props')
              v-icon(v-if='props.item.path === "/"', small, @click='goHome', color=`white`) mdi-home
              v-btn.ma-0(v-else, :href='props.item.path', small, text) {{props.item.name}}
          template(v-if='!isPublished')
            v-spacer
            .caption.red--text {{$t('common:page.unpublished')}}
            status-indicator.ml-3(negative, pulse)
      v-container.pa-0(fluid, :class='$vuetify.theme.dark ? `darken-4-l3` : `blue-light`')
        v-row(no-gutters, align-content='center', style='height: 90px;')
          v-col.page-col-content.is-page-header(offset-xl='2', offset-lg='3', style='margin-top: auto; margin-bottom: auto;', :class='$vuetify.rtl ? `pr-4` : `pl-4`')
            .headline.white--text(:class='$vuetify.theme.dark ? `text--lighten-2` : `text--darken-1`') {{title}}
            .caption.grey--text.text--darken-1 {{description}}
      v-divider
      v-container.blue-dark.pl-5.pt-4(fluid, grid-list-xl)
        v-layout.blue-dark(row)
          v-flex.page-col-sd(lg3, xl2, v-if='$vuetify.breakpoint.lgAndUp')
            v-card.sidebar-blue(style='border-radius: 12px;')
              v-card.no-shadow(v-if='tocDecoded.length', color='#333347')
                .overline.pa-5.pb-0(:class='$vuetify.theme.dark ? `blue--text text--lighten-2` : `light--text`') {{$t('common:page.toc')}}
                v-list.pb-3#page-contents(dense, nav, :class='$vuetify.theme.dark ? `darken-3-d3` : ``')
                  template(v-for='(tocItem, tocIdx) in tocDecoded')
                    v-list-item(@click='$vuetify.goTo(tocItem.anchor, scrollOpts)')
                      v-icon(color='grey', small) {{ $vuetify.rtl ? `mdi-chevron-left` : `mdi-chevron-right` }}
                      v-list-item-title.white--text.px-3 {{tocItem.title}}
                    //- v-divider(v-if='tocIdx < toc.length - 1 || tocItem.children.length')
                    template(v-for='tocSubItem in tocItem.children')
                      v-list-item(@click='$vuetify.goTo(tocSubItem.anchor, scrollOpts)')
                        v-icon.px-3(color='grey lighten-1', small) {{ $vuetify.rtl ? `mdi-chevron-left` : `mdi-chevron-right` }}
                        v-list-item-title.px-3.caption.grey--text(:class='$vuetify.theme.dark ? `text--lighten-1` : ``') {{tocSubItem.title}}
                      //- v-divider(inset, v-if='tocIdx < toc.length - 1')
              v-divider
              v-card.no-shadow(v-if='tags.length > 0', color='#333347')
                .pa-5
                  .overline.grey--text.pb-2(:class='$vuetify.theme.dark ? `text--lighten-3` : ``') {{$t('common:page.tags')}}
                  v-chip.mr-1.mb-1(
                    label
                    :color='$vuetify.theme.dark ? `teal darken-1` : `blue darken-3`'
                    v-for='(tag, idx) in tags'
                    :href='`/t/` + tag.tag'
                    :key='`tag-` + tag.tag'
                    )
                    v-icon(:color='$vuetify.theme.dark ? `teal lighten-3` : `white`', left, small) mdi-tag
                    span(:class='$vuetify.theme.dark ? `teal--text text--lighten-5` : `white--text`') {{tag.title}}
                  v-chip.mr-1.mb-1(
                    label
                    :color='$vuetify.theme.dark ? `teal darken-1` : `white`'
                    :href='`/t/` + tags.map(t => t.tag).join(`/`)'
                    :aria-label='$t(`common:page.tagsMatching`)'
                    )
                    v-icon(:color='$vuetify.theme.dark ? `teal lighten-3` : `blue darken-1`', size='20') mdi-tag-multiple
              v-card
              .pa-5
                .overline.grey--text.d-flex(:class='$vuetify.theme.dark ? `text--lighten-3` : ``')
                  ...
                  v-tooltip(right, v-if='isAuthenticated')
                    template(v-slot:activator='{ on }')
                      v-btn.btn-animate-edit(
                        ...
                        v-icon(color='blue darken-1', dense) mdi-history
                    span {{$t('common:header.history')}}
                .body-2.white--text(:class='$vuetify.theme.dark ? `` : `text--darken-3`') {{ authorName }}
                .caption.grey--text.text--darken-1 {{ updatedAt | moment('calendar') }}
              v-card(v-if='commentsEnabled && commentsPerms.read')
              .pa-5
                .overline.pb-2.grey--text.d-flex.align-center(:class='$vuetify.theme.dark ? `text--lighten-3` : ``')
                ...
                .d-flex
                  v-btn.text-none(
                    @click='goToComments()'
                    :color='$vuetify.theme.dark ? `blue-grey` : `blue darken-2`'
                    style='flex: 1 1 100%;'
                    small
                    )
                    span.white--text(:class='$vuetify.theme.dark ? `text--lighten-1` : `text--darken-2`') {{$t('common:comments.viewDiscussion')}}
              v-toolbar.blue-accent(:color='$vuetify.theme.dark ? `grey darken-4-d3` : ``', flat, dense)
                v-spacer
                v-tooltip(bottom)
                  template(v-slot:activator='{ on }')
          ...
          v-flex.page-col-content(xs12, lg9, xl10)
            ...
            .contents(ref='container')#no-copy
              slot(name='contents')
